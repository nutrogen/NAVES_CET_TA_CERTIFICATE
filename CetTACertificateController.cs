...
namespace NavesPortalforWebWithCoreMvc.Controllers.CET
{
    public class CetTACertificateController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> AnnexToCertPopup(TNAV_CET_TA_CERT tNAV_CET_TA_CERT)
        {
            try
            {
                if (tNAV_CET_TA_CERT is not null)
                {
                    Guid AnnexToCert_DefaultDbId;
                    DateTime _regTime = DateTime.Now;

                    if (tNAV_CET_TA_CERT.TA_CERT_IDX == Guid.Parse("00000000-0000-0000-0000-000000000000"))
                    {
                        AnnexToCert_DefaultDbId = Guid.NewGuid();
                    }
                    else
                    {
                        AnnexToCert_DefaultDbId = tNAV_CET_TA_CERT.TA_CERT_IDX;
                    }

                    TNAV_CET_TA_CERT _tNAV_CET_TA_CERT = new TNAV_CET_TA_CERT()
                    {
                        TA_CERT_IDX = AnnexToCert_DefaultDbId,
                        WORK_IDX = AnnexToCert_DefaultDbId,
                        WORK_ID = tNAV_CET_TA_CERT.WORK_ID,
                        ANNEX_TO_CERT = tNAV_CET_TA_CERT.ANNEX_TO_CERT,
                        IS_DELETE = false,
                        CREATE_USER = HttpContext.Session.GetString("UserName"),
                        REG_DATE = _regTime,
                        DELETE_DATE = tNAV_CET_TA_CERT.DELETE_DATE,
                        DELETE_USER = tNAV_CET_TA_CERT.DELETE_USER
                    };

                    _repository.Add(_tNAV_CET_TA_CERT);
                    await _repository.SaveChangesAsync();
                }

                return RedirectToAction(nameof(InsertModifyPopup), new { id = tNAV_CET_TA_CERT.TA_CERT_IDX });
            }
